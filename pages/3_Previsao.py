...
st.markdown('''
        Este gráfico apresenta a evolução histórica dos preços do petróleo Brent, separando os dados em dois conjuntos: treinamento (em azul) e teste real (em verde). A linha vermelha representa as previsões feitas pelo modelo XGBoost para o período de teste. O objetivo é mostrar como o modelo consegue aprender o comportamento dos preços passados e prever valores futuros com base nos dados históricos, permitindo avaliar visualmente a precisão das previsões em relação aos valores reais observados.
    ''')
...
st.markdown('''
        Neste gráfico, além das séries de treinamento (azul), teste real (verde) e previsão do modelo para o teste (vermelho), é apresentada também a projeção dos preços do petróleo Brent para os próximos 90 dias (linha laranja). Essa projeção é feita utilizando o modelo XGBoost com variáveis de defasagem (lags), que considera os últimos valores conhecidos para estimar os preços futuros. Assim, o gráfico ilustra tanto a capacidade do modelo em prever dados históricos quanto sua utilidade para antecipar tendências futuras do mercado de petróleo.
    ''')
